...
function [timings, stances, segmented] = REID_IMU_AminianODonovan(data, location, Fs, max_step_freq, min_stance_t)
...
for step_count = 1:size(ms_a_1_9_ind,2)
    ...
    % Therefore, we do not recommend this method for running data
    % When stepping forward in time ensure we don't go outside data window
    if size(a_1_9,2) - (ms_a_1_9_ind(step_count)+round(0.25*Fs_Aminian)) < 3
        % The window doesn't exist in our data so NaN-flag
        local_min_a_1_9_mag{step_count} = NaN;
        local_min_a_1_9_ind{step_count} = NaN;
    elseif size(a_1_9,2) - (ms_a_1_9_ind(step_count)+round(2*Fs_Aminian)) >= 0
        % The entire window exists in our data
        [local_min_a_1_9_mag{step_count} local_min_a_1_9_ind{step_count}] = findpeaks(-a_1_9(ms_a_1_9_ind(step_count)+round(0.25*Fs_Aminian):ms_a_1_9_ind(step_count)+round(2*Fs_Aminian)),'MinPeakHeight',0);
        % If there's no peaks
        if isempty(local_min_a_1_9_ind{step_count})
            % Then find the lowest value (max to be consistent with -signal used in find peaks)
            [local_min_a_1_9_mag{step_count} local_min_a_1_9_ind{step_count}] = max(-a_1_9(ms_a_1_9_ind(step_count)+round(0.25*Fs_Aminian):ms_a_1_9_ind(step_count)+round(2*Fs_Aminian)));
        end % if no peaks
    else
        % The start of the window exists in our data but the end point does not
        [local_min_a_1_9_mag{step_count} local_min_a_1_9_ind{step_count}] = findpeaks(-a_1_9(ms_a_1_9_ind(step_count)+round(0.25*Fs_Aminian):end),'MinPeakHeight',0);
        % If there's no peaks
        if isempty(local_min_a_1_9_ind{step_count})
            % Then find the lowest value (max to be consistent with -signal used in find peaks)
            [local_min_a_1_9_mag{step_count} local_min_a_1_9_ind{step_count}] = max(-a_1_9(ms_a_1_9_ind(step_count)+round(0.25*Fs_Aminian):end));
        end % if no peaks
    end % If window is outside data range
    local_min_a_1_9_ind{step_count} = local_min_a_1_9_ind{step_count} + ms_a_1_9_ind(step_count) + round(0.25*Fs_Aminian) - 1;
    ...
    if ms_a_3_9_ind(step_count)-round(0.05*Fs_Aminian) < 1 ||...
            ms_a_3_9_ind(step_count)-round(0.05*Fs_Aminian) < 3
        % The window doesn't exist in our data so NaN-flag
        local_min_a_3_9_mag{step_count}= NaN; local_min_a_3_9_ind{step_count} = NaN;
    elseif ms_a_3_9_ind(step_count)-round(2*Fs_Aminian) < 1
        % The start exists but the end doesn't
        [local_min_a_3_9_mag{step_count} local_min_a_3_9_ind{step_count}] = findpeaks(-a_3_9(1:ms_a_3_9_ind(step_count)-round(0.05*Fs_Aminian)),'MinPeakHeight',0);
        % If there's no peaks
        if isempty(local_min_a_3_9_ind{step_count})
            % Then find the lowest value (max to be consistent with -signal used in find peaks)
            [local_min_a_3_9_mag{step_count} local_min_a_3_9_ind{step_count}] = max(-a_3_9(1:ms_a_3_9_ind(step_count)-round(0.05*Fs_Aminian)));
        end % if no peaks
    else
        % The entire window exists
        [local_min_a_3_9_mag{step_count} local_min_a_3_9_ind{step_count}] = findpeaks(-a_3_9(ms_a_3_9_ind(step_count)-round(2*Fs_Aminian):ms_a_3_9_ind(step_count)-round(0.05*Fs_Aminian)),'MinPeakHeight',0);
        % If there's no peaks
        if isempty(local_min_a_3_9_ind{step_count})
            % Then find the lowest value (max to be consistent with -signal used in find peaks)
            [local_min_a_3_9_mag{step_count} local_min_a_3_9_ind{step_count}] = max(-a_3_9(ms_a_3_9_ind(step_count)-round(2*Fs_Aminian):ms_a_3_9_ind(step_count)-round(0.05*Fs_Aminian)));
        end % if no peaks
        local_min_a_3_9_ind{step_count} = local_min_a_3_9_ind{step_count} + ms_a_3_9_ind(step_count) - round(2*Fs_Aminian) - 1;
    end % If window is outside data range
    ...
    if any(~isnan(local_min_data_resample_ind{step_count})) && any(~isnan(local_min_a_1_9_ind{step_count}))
        ...
        while IC_identified == 0 && potential_IC_tick <= size(local_min_a_1_9_ind{step_count},2)
            ...
            while TC_identified == 0 && potential_TC_tick > 0
                if 0.1*Fs_Aminian < local_min_a_1_9_ind{step_count}(potential_IC_tick) - local_min_data_resample_ind{step_count}(potential_TC_tick) &&...
                        local_min_a_1_9_ind{step_count}(potential_IC_tick) - local_min_data_resample_ind{step_count}(potential_TC_tick) < 2.5*Fs_Aminian
                ...
                end % 0.1 s < (IC - TC) < 2.5 s
            end % TC_identified = 0
            potential_IC_tick = potential_IC_tick + 1;
        end % IC_identified = 0
    end % data in potential IC and TC arrays
end % step_count
