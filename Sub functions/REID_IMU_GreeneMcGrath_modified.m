%% REID_IMU_GreeneMcGrath_modified
...
% The modified version presented here adjusts the temporal windows reported
% in the original algorithm to better correspond to running data

function [timings, stances, segmented] = REID_IMU_GreeneMcGrath_modified(data, location, Fs, max_step_freq, max_swing_t, min_stance_t)
...
% Modified to better correspond to the time between mid-swings that might
% be expected in running
t1 = round(Fs/max_step_freq)*2;
...
if ~isempty(potential_midswing_ind)
    ...
    for step_count = size(potential_midswing_ind,1):-1:1
        ...
        % Modified to better correspond to swing time that might be expected
        % during running
        t2 = round(max_swing_t/1000/2*Fs);
        % However, if this exceeds the size of the data set just look to the end
        if potential_midswing_ind(step_count)+t2 > size(ML_filt,1)
            t2_temp = size(ML_filt,1) - potential_midswing_ind(step_count);
        else
            t2_temp = t2;
        end
        [potential_IC_mag potential_IC_ind] = findpeaks(-ML_filt(potential_midswing_ind(step_count):potential_midswing_ind(step_count)+t2_temp));
        if isempty(potential_IC_ind)
            [potential_IC_mag potential_IC_ind] = max(-ML_filt(potential_midswing_ind(step_count):potential_midswing_ind(step_count)+t2_temp));
        end
